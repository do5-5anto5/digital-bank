...
class GetMask {
    ...
    companion object {
        ...
        fun getFormattedDate(date: Long, type: Int): String {
            ...
            val fuse = "America/Sao_Paulo"

            val daySdf = SimpleDateFormat("dd", locale)
            daySdf.timeZone = TimeZone.getTimeZone(fuse)

            val monthSdf = SimpleDateFormat("MM", locale)
            monthSdf.timeZone = TimeZone.getTimeZone(fuse)

            val yearSdf = SimpleDateFormat("yyyy", locale)
            yearSdf.timeZone = TimeZone.getTimeZone(fuse)

            val hourSdf = SimpleDateFormat("HH", locale)
            hourSdf.timeZone = TimeZone.getTimeZone(fuse)

            val minuteSdf = SimpleDateFormat("mm", locale)
            minuteSdf.timeZone = TimeZone.getTimeZone(fuse)
            ...
            val time: String = when (type) {
                ...
                DAY_MONTH -> "$day/$month"
                else -> R.string.generic_error.toString()
            }
            return time
        }
